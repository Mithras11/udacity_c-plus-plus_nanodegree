...
T findValueByKey(const std::string& pathName, const std::string& keyFilter) {
  ...
  while (std::getline(filestream, line)) {
    ...
    if (linestream >> key && key == keyFilter) {
      if (!(linestream >> value)) {
        throw std::runtime_error("Failed to extract value for key " +
                                 keyFilter);
      }
      return value;
    }
  ...
  throw std::runtime_error("Key " + keyFilter + " not found in file " +
                           pathName);
}
...
long LinuxParser::ActiveJiffies(int pid) {
  ...
  return (activeJiffies * 100) / totalJiffies;
}

// Read and return the number of active jiffies for the system
long LinuxParser::ActiveJiffies() {
  vector<string> cpu_utilization = CpuUtilization();
  vector<int> active_jiffies_indexes = {kUser_, kNice_,    kSystem_,
...
long LinuxParser::IdleJiffies() {
  vector<string> cpu_utilization = CpuUtilization();
  return stof(cpu_utilization[kIdle_]) + stof(cpu_utilization[kIOwait_]);
}
    ...
    while (getline(filestream, line)) {
      ...
      if (cpu_label == "cpu") {
        std::copy(std::istream_iterator<string>(linestream),
                  std::istream_iterator<string>(), std::back_inserter(jiffies));
      }
    }
  }
  return jiffies;
...
long LinuxParser::UpTime(int pid) {
  ...
  try {
    if (!tokens.empty()) {
      ...
      return system_uptime - uptime;  // subtract process starttime from system
                                      // uptime to get process uptime
    }
